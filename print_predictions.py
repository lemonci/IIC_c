...
dataset = torchvision.datasets.ImageFolder('/home/monica/Shicang/Resize_white/train', transform = torchvision.transforms.ToTensor())
data = []
for i in range(len(dataset)):
#  print dataset[i][0].shape
  #new_cha = dataset[i][0][0].unsqueeze(0)
  stack_data = torch.stack([dataset[i][0][0], dataset[i][0][1],dataset[i][0][2],dataset[i][0][0]], dim=0)
  #print cat_data.shape
  data.append((stack_data.unsqueeze(0), dataset[i][1]))
...
#  if len(inputs.shape)[0] == 3: inputs = inputs.unsqueeze(0) # shape (c, h, w) ->  (1, c, h, w)
